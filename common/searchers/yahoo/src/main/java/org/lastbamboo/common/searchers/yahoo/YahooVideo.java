 ...
 */
public class YahooVideo extends AbstractRestResult
    {

    private static final Logger LOG = LoggerFactory.getLogger(YahooVideo.class);
    
    private static final XPathFactory s_xPathFactory = 
        XPathFactory.newInstance();
    private static final XPath s_xPath = s_xPathFactory.newXPath();
    
    private static final String YAHOO_VIDEO_NS = "urn:yahoo:srchmv";
    
    static
        {
        s_xPath.setNamespaceContext(new YahooNamespaceContext(YAHOO_VIDEO_NS));
        }
    
    /**
     * Creates a new video.
     * 
     * @param element The XML element to extract video data from.
     * @param index The index of this result in the parent document.
     * @throws URISyntaxException If there are any URI parsing errors in the
     * data.
     */
    public YahooVideo(final Element element, final int index) 
     ...
     * URL format.
     */
    public static URI createThumbnailUrl(final Element element, final int index) 
        throws URISyntaxException
        {        
    ...
    private static int toInt(final String xPathSuffix, final Element element, 
        ...
        try
            ...
            stringVal = (String) s_xPath.evaluate(xPath, element, 
                XPathConstants.STRING);
            if (!NumberUtils.isNumber(stringVal))
                {
                // We just return a generic number fit for most thumbnails.
                return 75;
                }
