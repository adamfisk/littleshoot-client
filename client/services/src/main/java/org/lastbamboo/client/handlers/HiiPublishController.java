...
/**
 * Controller for publishing files from an external desktop application.
 */
public class HiiPublishController extends HttpServlet {

    private static final long serialVersionUID = -7594135886375530730L;
    ...
    protected void doGet(final HttpServletRequest request, 
        ...
        final String key = paramMap.get(ShootConstants.HII_KEY);
        if (!key.equalsIgnoreCase(localKey)) {
            System.out.println(key + " not equal to "+ localKey);
            invalidRequest(request, response, 
                "Could not publish. Security error.");
            return;
        }
        else {
