...
static NPError _OpenURL(
    NPP npp, const char * szURL, const char * szTarget, void * pNotifyData, 
    const char * pData, uint32 len, NPBool isFile
    )
{
    if (!npp)
    {
    ...
    PluginHostWnd *pWnd = (PluginHostWnd *) npp->ndata;
    ATLASSERT(pWnd);
