...
CommonUtils =
    ...
    showEmailCollectDialog : function ()
        {
        dijit.byId("emailCollectDialog").show();  
        },
    ...
    isValidEmail : function (email)
        ...
        if (!dojox.validate.isEmailAddress(email))
            ...
            CommonUtils.showMessage("Not an E-mail Address", 
                "You didn't enter an e-mail address.  Please try again.");
            return false;
            }
        else
            {
    ...
    editFile : function()
        {
        CommonUtils.keyFormRequest('editFileDialog', 'editFile', CommonUtils.loadFiles);
        },
    ...
    keyFormRequest : function (dialogId, path, onSuccess)
        {
        ...
        var success = dojo.hitch(this, function (response)
            ...
            CommonUtils.signFormRequest(dialogId, path, onSuccess, id);
            });
    ...
    signFormRequest : function(dialogId, path, onSuccess, keyId)
        {
        if (!path)
            {
        ...
        //console.info("Sending signed request: "+dialogId);
        var dialog = dijit.byId(dialogId);
        
        var url = Constants.CLIENT_SECURE_URL + "signedRelay";
        var params = dialog.getValues();
        params.path = path;
        params.keyId = keyId;
        
        //console.info("Got form values: "+dojo.toJson(params));
        
        var siteKey = dojo.cookie("siteKey");
        if (!siteKey)
            {
            console.error("No site key");
        ...
        var success = dojo.hitch(this, function (response)
            ...
            if (!response.success)
                {
                CommonUtils.showMessage("Submission Error", response.message);
                return;
                }
        ...
        var error = dojo.hitch(this, function (response)
            {
            console.error("Got error response: "+response);
            CommonUtils.showMessage("LittleShoot Error", 
                "We're sorry, but there was an error contacting LittleShoot.  " +
    ...
    endsWith : function (str, ending) {
        var index = str.length - ending.length;
        if (index < 0)
            {
            return false;
            }
        var strEnding = str.substring(index);
    ...
    get : function (getConfig)
        ...
        var deferred = dojo.io.script.get(getConfig);
        //CommonUtils.googleAnalytics(getConfig.url);
        return deferred;
        },
